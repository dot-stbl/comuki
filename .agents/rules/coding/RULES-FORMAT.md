...
## 2. `.agents/rules/<category>/*.md` — контекстные правила
...
1. `.agents/rules.local/` (per-project, gitignored, личные override'ы)
...
Например, личный override в `.agents/rules.local/` бьёт всё ниже.

## Где лежит что (текущее состояние)

> **Замечание про comuki.orchestrator:** исторически большие manual-файлы
> лежат в `.agents/rules/coding/` (а не в `.claude/rules/` как описано в §1
...
> старых ссылках — это устаревшее, актуальные правила — в `.agents/rules/coding/`.
...
его в `.agents/rules/`. Наоборот:

| Если правило... | Класть в |
|------------------|----------|
| Учит, развёрнутое, 100+ строк, ссылаются агенты в любом контексте | `.claude/rules/` |
| Короткое, контекстное, "когда касается X — делай Y" | `.agents/rules/<category>/` |
| Бланкетный запрет / обязанность, должен быть в эффекте всегда | `.agents/rules/<category>/` с `always: true` |
| Большое + должно быть в эффекте при касании конкретных файлов | TL;DR в `.agents/rules/`, полная версия в `.claude/rules/` |
| Большое + всегда нужно в контексте (comuki convention) | `.agents/rules/coding/` с `always: true` (НЕ `.claude/rules/`) |
...
### В `.agents/rules/<category>/` (контекстное)

1. Создать `.agents/rules/<category>/<name>.md`
...
После создания / редактирования правил в `.agents/rules/` обычно нужно

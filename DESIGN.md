...
### Status ladder
The seven statuses on the default theme are a lightness ladder with warm/cold alternation on the blue↔yellow axis — the axis dichromat vision preserves. `running` and `success` are held the widest berth on the ladder (ΔL\* 26) because they are the two statuses whose bands carry no weave — tone is their only code. `queued` sits nearest the zero-saturation point by the old design-system rule that it is the least saturated status; it and `waiting` share a side of the axis and are told apart by saturation and weave, which a test states explicitly.

`cancelled` is the seventh rung, and the palette chose where it went rather than a designer. Holding the floors already in force — 4.5:1 for a status label on the lane, 5 L\* between any two rungs — leaves exactly one place free in the whole gamut: one step past `success`, at 92.3 L\* in dark and 9.4 in light. Below `running` a label stops being readable on the lane (58.4 L\* is where that ends, and `running` already sits at 60.7), and the six rungs are 5.3 apart, so nothing fits between them either. What that leaves free is the axis that actually carries alarm on a colourless board: **chroma**. `failed` shouts at ±170 on the blue↔yellow offset and `waiting` at ±114; `cancelled` sits at ±28, second only to `queued`, warm, and near enough to neutral that it reads at body weight rather than as a signal. A cancelled run is terminal, deliberate and uninteresting, and that is what it looks like. It cannot be neutral outright — a zero offset lands 0.069 ΔEok from `success` and loses the separation floor — so its real second channel is the hatch, which is the sparsest in the set.
...
**The Two-Channel Status Rule.** A status is never hue alone. `[data-status="…"]` in `tokens.css` sets both `--hue` and `--weave`, and it is the only place in the product either property is declared — a component marks *what it paints* (`background-color: var(--hue)`) and never *which status* it is painting. Every wordless mark reads both: the flow band, the river's legend swatch, the collapsed strip, the run graph's node edge, the verdict band's rail. Two marks read the hue and refuse the weave on purpose — the status badge and the bar series, each of which writes the status out in the product's own word right beside itself and so already carries a channel that survives greyscale. Refusing a channel is a decision the declaration allows; inventing a value for one is not. Adding a status means adding a hatch, not just a colour, and it means editing that one block — which is what `cancelled` did.

The rule was written before it was true. Six modules each carried their own copy of the pair, in three spellings: two near-identical six-arm blocks in the profile river and the run graph, a third in the verdict band with a `clear` alias bolted on, three more that skipped `--hue` altogether and reached for `var(--st-…)` directly, and the status badge, which invented a private `--st` and set no weave at all. Nothing failed when two of them disagreed, because nothing compared them. A seventh status cost about fourteen files; it now costs the registry, its two tokens, one arm in `tokens.css` and the badge's glyph — and of the hand-written stylesheets, exactly one. That is no longer a claim. `cancelled` was added as the seventh, and `tokens.css` was the only stylesheet it touched. What it cost outside the sheets was the registry (a hue per theme, per mode), the `Status` union, the badge's glyph, the mapper that had been folding it onto `failed`, the two exhaustive records the compiler pointed at afterwards — the triage rank and the empty pool — and the two ordered lists it could not, because a `RunStatus[]` is not a `Record<RunStatus, …>`: the duty list's filter options and the river's segment order. Those two are the residue of the refactor, and finding them is a read rather than a build. `app/styles/status-channel.test.ts` is what holds it: it reads the sheets back off disk, runs the real selectors against the real DOM shapes, resolves every `var()` chain, and asserts the pair each consumer ends up with in both modes — none of which jsdom can paint and none of which is visible by eye.

**The Real Words Rule.** Status colours map one-to-one onto the product's seven statuses: running, success, failed, waiting, queued, escalated, cancelled. `cancelled` is the word the wire already sends; it spent a while being repainted as `failed` on the way in, which is the same defect as coining a synonym, pointed the other way. UI copy says "waiting on a human", never a coined synonym. Internal model names stay in the model and never surface as labels.
...
- **Semantics:** seven variants matching the seven statuses. `failed` is the one that escalates — 40% border and a full tint — because a failure should be visible before it is read. `cancelled` is the one at the other end: the quietest hue on the ladder and a bare stroke for a glyph, so a stopped run reads as a row that is over rather than a row that went wrong.
...
- **Do** write UI copy in the product's seven real status words.
...
- **`data-status` carries one word that is not a status.** The duty screen's verdict band says `clear` when nothing needs a person, and `tokens.css` gives `clear` success's hue and success's hatch so the attribute keeps a single vocabulary. It is an alias, not a status of its own — the Real Words Rule still holds, because `clear` is never a run's state and never appears as a status label — but the block that ought to read as exactly the seven reads as seven plus a borrowed word.
- **The hatch angles are full.** Every weave is at least 22.5° from every other, which is what lets two hatches be told apart at a glance, and `cancelled` took the last gap that clears it — 67.5°, between `escalated` at 45° and `queued` at 90°. An eighth status has no angle left at this floor. It would have to buy its second channel another way — a cross-hatch, a dash, a mark of a different kind — or the floor would have to come down. `status-channel.test.ts` asserts the 22.5° separation, so that wall is hit by a failing test rather than by a hatch nobody can read.
- **The run-outcomes band still stacks three statuses.** `cancelled` is terminal and is not one of them, because the day series behind that chart is seeded rather than served and the seed's own vocabulary predates the status. A fourth band would be a legend entry with no bar under it on every day of the week. It goes in with the endpoint.

...
const USER_LINE =
  '{"type":"user","message":{"role":"user","content":"Say hello in exactly one word"}}'
...
const UNKNOWN_LINE =
  '{"type":"some_future_event_type","data":{"hello":"world"}}'
...
describe("piEventSchema round-trips", () => {
  test.each([
    ...
    const roundTripped = piEventSchema.parse(
      JSON.parse(JSON.stringify(piEvent))
    )

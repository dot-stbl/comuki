...
describe("BLOCKED_TOOL_TARGETS", () => {
  ...
  test("blocks editing test files", () => {
    ...
    expect(
      findPathLock(BLOCKED_TOOL_TARGETS, "tests/unit/parser.test.ts")
    ).toBeDefined()
  ...
  test("blocks package installs", () => {
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(npm install zod)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(npm add zod)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(bun add @types/bun)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(bun install)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(pnpm install)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(yarn install)")
    ).toBeDefined()
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(pip install requests)")
    ).toBeDefined()
    expect(
      findToolLock(
    ...
    expect(
      findToolLock(BLOCKED_TOOL_TARGETS, "Bash(npm run build)")
    ).toBeUndefined()
  })

  test("blocks pushing to protected branches", () => {
    expect(
      findGitRefLock(BLOCKED_TOOL_TARGETS, "refs/heads/main")
    ).toBeDefined()
    expect(
      findGitRefLock(BLOCKED_TOOL_TARGETS, "refs/heads/master")
    ).toBeDefined()
    expect(
      findGitRefLock(BLOCKED_TOOL_TARGETS, "refs/heads/feature/agents-sdk")
    ).toBeUndefined()
  })
})

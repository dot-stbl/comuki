...
describe("find*Lock helpers", () => {
  ...
  test("findPathLock returns the first matching rule", () => {
    expect(findPathLock(rules, "src/foo.test.ts")?.id).toBe(
      "test-no-edit-tests"
    )
  ...
  test("findToolLock returns the first matching rule", () => {
    expect(findToolLock(rules, "Bash(bun add left-pad)")?.id).toBe(
      "test-no-bun-add"
    )
  ...
  test("findGitRefLock returns the first matching rule", () => {
    expect(findGitRefLock(rules, "refs/heads/main")?.id).toBe(
      "test-no-push-main"
    )

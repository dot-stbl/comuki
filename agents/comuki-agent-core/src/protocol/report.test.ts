...
describe("parseReport", () => {
  ...
  test("parses a minimal failed report", () => {
    const report = parseReport({
      status: "failed",
      summary: "Build failed: type error",
    })
  ...
  test("rejects an unknown status", () => {
    expect(() =>
      parseReport({ status: "running", summary: "still going" })
    ).toThrow()
  })
  ...
  test("schema round-trips a valid report", () => {
    const report = parseReport({
      status: "cancelled",
      summary: "Stopped by orchestrator",
    })

    expect(reportSchema.parse(JSON.parse(JSON.stringify(report)))).toEqual(
      report
    )
  })
})

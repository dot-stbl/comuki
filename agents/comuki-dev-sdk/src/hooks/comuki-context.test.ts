...
function recordingFetch(handler: (url: string) => Response | null): {
  fetchImpl: FetchFn
  urls: string[]
} {
  const urls: string[] = []
...
describe("url builders", () => {
  test("profiles catalog url strips trailing slashes", () => {
    expect(profilesCatalogUrl("http://localhost:17173")).toBe(
      "http://localhost:17173/profiles"
    )
    expect(profilesCatalogUrl("http://localhost:17173///")).toBe(
      "http://localhost:17173/profiles"
    )
  ...
  test("memory digest url encodes the project name", () => {
    expect(memoryDigestUrl("http://host", "my project")).toBe(
      "http://host/api/v1/memory/digest?project=my%20project"
    )
    expect(memoryDigestUrl("http://host")).toBe(
      "http://host/api/v1/memory/digest"
    )
  })
})
...
describe("buildSessionContext", () => {
  ...
  test("host adds catalog pointer and fetches the digest", async () => {
    const { fetchImpl, urls } = recordingFetch(() =>
      okResponse('{"summary":"recent decisions"}')
    )

    const context = await buildSessionContext(
      { COMUKI_PROJECT: "anlytra", COMUKI_HOST: "http://localhost:17173" },
      fetchImpl
    )

    expect(context).toContain(
      "- Profiles catalog: http://localhost:17173/profiles"
    )
    expect(context).toContain("### Memory digest")
    expect(context).toContain('{"summary":"recent decisions"}')
    expect(urls).toEqual([
      "http://localhost:17173/api/v1/memory/digest?project=anlytra",
    ])
  })

  test("mcp url is listed when set", async () => {
    const context = await buildSessionContext({
      COMUKI_MCP_URL: "http://host/mcp",
    })
  ...
  test("digest is capped at MAX_DIGEST_LENGTH", async () => {
    ...
    const context = await buildSessionContext(
      { COMUKI_HOST: "http://localhost:17173" },
      fetchImpl
    )

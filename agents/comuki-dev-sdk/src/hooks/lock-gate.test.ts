...
describe("decideLockGate — edit-path locks", () => {
  ...
  test("denies writing spec files and nested test dirs", () => {
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Write", { file_path: "deep/pkg/foo.spec.tsx", content: "" })
        )
      ).decision
    ).toBe("deny")
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Edit", { file_path: "tests/unit/parser.test.js" })
        )
      ).decision
    ).toBe("deny")
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Edit", { file_path: "src/__tests__/helper.ts" })
        )
      ).decision
    ).toBe("deny")
  })
  ...
  test("denies NotebookEdit via notebook_path", () => {
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("NotebookEdit", { notebook_path: "notes/tests.ipynb" })
        )
      ).decision
    ).toBe("allow")
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("NotebookEdit", { notebook_path: "tests/notes.ipynb" })
        )
      ).decision
    ).toBe("deny")
  })
  ...
  test("allows editing regular source files", () => {
    expect(
      decideLockGate(
        parseHookPayload(hookJson("Edit", { file_path: "src/foo.ts" }))
      ).decision
    ).toBe("allow")
  })
  ...
  test("normalizes windows separators in file paths", () => {
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Edit", { file_path: "src\\deep\\foo.test.ts" })
        )
      ).decision
    ).toBe("deny")
  })
...
describe("decideLockGate — tool-name locks (installs)", () => {
  ...
  test.each(installCommands)('denies "%s"', (command) => {
    expect(
      decideLockGate(parseHookPayload(hookJson("Bash", { command }))).decision
    ).toBe("deny")
  })
  ...
  test("allows ordinary package-manager scripts", () => {
    expect(
      decideLockGate(
        parseHookPayload(hookJson("Bash", { command: "npm run build" }))
      ).decision
    ).toBe("allow")
    expect(
      decideLockGate(
        parseHookPayload(hookJson("Bash", { command: "bun test" }))
      ).decision
    ).toBe("allow")
  })
})
...
describe("gitPushRefCandidates", () => {
  ...
  test("skips flags, flag values and global git flags", () => {
    expect(gitPushRefCandidates("git push --force origin main")).toContain(
      "refs/heads/main"
    )
    expect(gitPushRefCandidates("git -C sub push origin main")).toContain(
      "refs/heads/main"
    )
    expect(
      gitPushRefCandidates("git push --repo=upstream origin main")
...
describe("decideLockGate — git-ref locks (push to protected branches)", () => {
  ...
  test.each(protectedPushes)('denies "%s"', (command) => {
    const decision = decideLockGate(
      parseHookPayload(hookJson("Bash", { command }))
    )

    expect(decision.decision).toBe("deny")
    expect(
      decision.ruleId !== undefined &&
        ["no-push-main", "no-push-master"].includes(decision.ruleId)
    ).toBe(true)
  })
  ...
  test("allows pushes to feature branches and non-push mentions", () => {
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Bash", { command: "git push origin feature/dev-sdk" })
        )
      ).decision
    ).toBe("allow")
    expect(
      decideLockGate(
        parseHookPayload(
          hookJson("Bash", { command: 'git commit -m "push main now"' })
        )
      ).decision
    ).toBe("allow")
  })
...
describe("decideLockGate — fail-open on unknown shapes", () => {
  test("allows on malformed json", () => {
    expect(decideLockGate(parseHookPayload("not json at all")).decision).toBe(
      "allow"
    )
  })
  ...
  test("allows unknown tools", () => {
    expect(
      decideLockGate(
        parseHookPayload(hookJson("Read", { file_path: "src/foo.test.ts" }))
      ).decision
    ).toBe("allow")
  })
  ...
  test("allows Bash without a command string", () => {
    expect(
      decideLockGate(parseHookPayload(hookJson("Bash", { command: 42 })))
        .decision
    ).toBe("allow")
  })
...
describe("emitDecision — both output shapes behind one emitter", () => {
  ...
  test("exit style: allow is exit 0 and silent", () => {
    expect(emitDecision(allowGate, "exit")).toEqual({
      exitCode: 0,
      stdout: "",
      stderr: "",
    })
...
describe("runLockGate — stdin fixtures end to end", () => {
  test("denied edit produces json decision by default", () => {
    const output = runLockGate(
      hookJson("Edit", { file_path: "src/app.test.ts" })
    )
  ...
  test("empty env keeps json default", () => {
    const output = runLockGate(
      hookJson("Edit", { file_path: "src/app.ts" }),
      {}
    )

...
export function isOurCommand(command: string, packageDir: string): boolean {
  ...
  const normalizedPackageDir = packageDir
    .replaceAll("\\", "/")
    .replace(/\/+$/, "")
...
export function applyHooks(
  ...
  for (const def of defs) {
    ...
    const entry: MutableHookCommandEntry = {
      type: "command",
      command: def.command,
      timeout: def.timeout,
    }
    ...
    const keptGroups = groups
      .map((group) => ({
        ...
        hooks: (group.hooks ?? []).filter(
          (entry) => !isOurCommand(entry.command, packageDir)
        ),
      }))
...
export async function installHooks(options: {
  ...
  const next = applyCoAuthoredBy(
    applyHooks(current, options.defs ?? buildHookDefs(options.packageDir))
  )
...
if (import.meta.main) {
  ...
  const settingsPath =
    settingsFlagIndex !== -1
      ? (args[settingsFlagIndex + 1] ?? DEFAULT_SETTINGS_PATH())
  ...
  if (!result.changed) {
    process.stdout.write(
      uninstall
        ? "comuki hooks: nothing to remove\n"

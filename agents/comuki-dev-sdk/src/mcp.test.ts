 ...
 * DELETE on close.
 */
function fakeStreamableServer(options: { readonly failWith?: number } = {}): {
  fetchImpl: FetchFn
  ...
  const fetchImpl: FetchFn = async (url, init) => {
    ...
    const body =
      bodyText.length > 0
        ? (JSON.parse(bodyText) as { method?: string; id?: number | string })
        : undefined
    requests.push({
      ...
      authorization:
        init?.headers instanceof Headers
          ...
          : null,
      accept:
        init?.headers instanceof Headers ? init.headers.get("accept") : null,
      body,
    })
...
describe("fromEnv", () => {
  ...
  test("builds a client from env with optional token", async () => {
    const instance = ComukiMcpClient.fromEnv({
...
describe("connect over streamable http", () => {
  test("connects, sends bearer auth and negotiates initialize", async () => {
    ...
    const initialize = server.requests.find(
      (request) =>
        (request.body as { method?: string } | undefined)?.method ===
        "initialize"
    )
  ...
  test("callTool passes arguments and maps the result", async () => {
    ...
    expect(
      (call?.body as { params?: { arguments?: { query: string } } })?.params
        ?.arguments
    ).toEqual({ query: "locks" })
  })
})
...
describe("fail-soft behaviour", () => {
  ...
  test("server rejecting streamable falls back to the SSE transport before giving up", async () => {
    ...
    const sseAttempt = server.requests.find(
      (request) =>
        request.method === "GET" &&
        request.accept?.includes("text/event-stream") === true
    )

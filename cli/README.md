...
сессия, `/stop` — прервать текущий ход (клиентский abort: сервер
продолжает думать, CLI перестаёт слушать и ставит dim-метку
`⏺ stopped`), `approve` / `reject [reason]` — решение по ждущему плану.
При вводе `/` над промптом открывается меню автокомплита: фильтрация
на лету, `↑`/`↓` выбор, `tab`/`enter` дополнение, `esc` закрыть.

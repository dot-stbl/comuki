/**
 * Transcript flattening tests: blocks → lines order, approval card,
 * typing spinner + live tail, notices, and the ANSI-aware width wrap
 * that keeps the fixed-height viewport honest.
 */
...
describe("flattenTranscript", () => {
  ...
  test("message blocks and line blocks keep transcript order", () => {
    ...
    expect(plain[0]).toContain("you")
    expect(plain[0]).toContain("hi there")
  ...
  test("awaiting approval appends the plan and the hint", () => {
    const plan = {
      nodes: [{ key: "n1", profileKey: "default", brief: "do things\nmore", dependsOn: [] }],
    }
    ...
    const plain = lines.map(stripAnsi)
    expect(plain.some((line) => line.includes("default"))).toBe(true)
    expect(plain.some((line) => line.includes("do things"))).toBe(true)
    expect(plain[plain.length - 1]).toContain("approve or reject")
    expect(plain[plain.length - 1]).toBe(APPROVAL_HINT)
  })

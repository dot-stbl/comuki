...
async function main() {
  ...
  const storyItems = await page.evaluate(() => {
    const items = document.querySelectorAll(
      '[id^="storybook-explorer-treeitem-"]'
    )
    return Array.from(items)
      .map((item) => {
        const id = (item as HTMLElement).id.replace(
          "storybook-explorer-treeitem-",
          ""
        )
  ...
  writeFileSync(
    ...
    [
      ...
      ...failed.map(
        (r) =>
          `  - ${r.id}: console=${r.consoleErrors.length} page=${r.pageErrors.length}`
      ),
      ``,

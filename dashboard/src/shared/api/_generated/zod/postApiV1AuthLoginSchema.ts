/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/

import { loginRequestSchema } from "./loginRequestSchema";
import { loginResponseSchema } from "./loginResponseSchema";
import { problemDetailsSchema } from "./problemDetailsSchema";
import { z } from "zod/v4";

/**
 * @description OK
 */
export const postApiV1AuthLogin200Schema = z.lazy(() => loginResponseSchema)

/**
 * @description Unauthorized
 */
export const postApiV1AuthLogin401Schema = z.lazy(() => problemDetailsSchema)

export const postApiV1AuthLoginMutationRequestSchema = z.lazy(() => loginRequestSchema)

export const postApiV1AuthLoginMutationResponseSchema = z.lazy(() => postApiV1AuthLogin200Schema)
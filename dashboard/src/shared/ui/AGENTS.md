...
## 3a. Формы — две вещи, которые кит уже решил, и решать заново нельзя

**Когда показывается ошибка поля — `touched || attempted`.** Одна модель на
продукт. `touched` — оператор был в поле и вышел (он закончил ответ, значит
сказать «неверно» — это ответ на его вопрос; говорить во время набора — это
перебивать на полуслове). `attempted` — он нажал кнопку (все правила, которые
форма держала, теперь должны быть отданы сразу, включая поля, куда он не
заходил). Показанная ошибка **не прячется** до исправления. Эталон —
`domains/projects/ui/create-project-form.tsx`, контракт — доккоммент
`FieldProps.error` в [`form/field.tsx`](form/field.tsx). Четвёртая модель —
дефект, не вкус. Валидность живёт на краю формы (схема), не в поле.

**`aria-*` вызывающего сливаются, не затираются.** Поля спредят `{...rest}`
и потом ставят свои `aria-describedby` / `aria-invalid` / `aria-required`.
Значения **объединяются** (`describedBy()` в [`form/ids.ts`](form/ids.ts)), а не
выбирается победитель: порядок спреда в любую сторону просто меняет, кто
проигрывает. `aria-describedby` — список; экран снаружи (баннер провала логина)
и поле внутри (своя строка ошибки) верны одновременно. Не гонять id через
`cn()` — это `twMerge`, он парсит вход как классы Tailwind и вправе выкинуть то,
что счёл перекрытым.

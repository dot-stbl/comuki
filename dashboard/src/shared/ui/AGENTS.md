...
- **Токены** — `app/styles/tokens.css` (импорт из `src/index.css`).
  Нет нужного токена → добавь в `tokens.css`. **Никогда** не хардкодь hex/px.
...
- Только `var(--token)` из `app/styles/tokens.css`.

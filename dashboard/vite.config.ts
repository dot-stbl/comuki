...
export default defineConfig(() => {
  return {
    plugins: [
      ...
      react(),
      tailwindcss(),
      // One-line startup banner so the operator can tell at a glance which
      // mode Vite booted into — mock seeds vs the host's real API. The
      // gating mirrors the `useMock` transform in `src/shared/config/env.ts`
      // (undefined / empty / "true" / "1" all read as mock), and prints the
      // effective mode + the resolved API URL (or "(empty)"). It is the only
      // place a `vite --mode …` choice is echoed back at startup — the badge
      // in the topbar is the in-app mirror.
      {
        name: "comuki-mock-mode-banner",
        configResolved(config) {
          const raw = config.env.VITE_USE_MOCK
          const useMock =
            config.mode === "mock" ||
            raw === undefined ||
            raw === "" ||

...
Defines tracker integration ingress into Comuki: anonymous signed webhooks,
source connections, admission rules (watch vs inbox), the human inbox/claim
loop, native inbound item creation, and sync-back jobs that push status to
trackers. This capability is the hard pre-release replacement for the
retired bounded context — identical behavior, renamed schema, entities,
routes, and permissions; no compatibility aliases carried forward.
...
- **THEN** the answer is 401 ProblemDetails and no inbound item is admitted
...
Integrations SHALL support kebab-case provider keys `github`, `gitlab`, `jira`, and
...
canonical inbound item shape (project, key, title, body, reporter, status, labels),
and create an `inbound_items` row on success. Both providers SHALL expose a
...
#### Scenario: Yandex Tracker webhook creates an inbound item
...
- **THEN** the verifier accepts the signature, the mapper admits the
  payload, and a row in `inbound_items` is created with `provider =
  "yandex-tracker"` and the ticket's external id

#### Scenario: Jira webhook creates an inbound item
...
- **THEN** the create is rejected with 400 ProblemDetails (code
  `integration.provider_settings_invalid`) and the row is not stored
...
REST under `/api/v1/integration/sources` SHALL demand `integration:read` for reads and
...
### Requirement: Inbound item kind discriminator
Admitted inbound items SHALL carry an `InboundItemKind` discriminator with
...
on every admitted inbound item; pre-existing rows SHALL be backfilled to
`Issue`.

#### Scenario: GitHub pull request stamps PullRequest
- **WHEN** a GitHub `pull_request.opened` webhook admits an inbound item
- **THEN** `kind = PullRequest` is stamped at normalize time and the
  inbound item row carries the discriminator

#### Scenario: Plain issue stays Issue
- **WHEN** a GitHub `issues.opened` or any other tracker issue event
  admits an inbound item
- **THEN** `kind = Issue` and the value never changes for the inbound item's
  lifetime
...
`PullRequest`-kind inbound items on the same `InboundItem` shape (the
...
- **WHEN** a signed `pull_request` webhook with action `opened`
  arrives
- **THEN** the mapper admits a `PullRequest`-kind inbound item and the
  webhook pipeline behaves like an issue admission (delivery lock,
  signature verify, normalize, admission, inbound item insert, run launch)

#### Scenario: GitLab merge_request open admission
- **WHEN** a signed `merge_request` webhook with action `open`
  arrives
- **THEN** the mapper admits a `PullRequest`-kind inbound item
...
Admitted inbound items SHALL land on a profile chosen by
`IIntegrationProfileRouter`. The integrations module SHALL expose
`IIntegrationProfileRouter` as a port that maps an admitted inbound item to a
...
`pr-review` for `PullRequest`-kind inbound items, and SHALL fall back to
`Integrations:Worker:IssueDefaultProfileKey` (default `general`) for
`Issue`-kind inbound items. The router SHALL never throw — broken JSON,
missing fields, and non-string values silently use the fallback.
Admitted PR inbound items SHALL NOT claim on the `implement` profile. The
integrations module SHALL NOT depend on the engine — the host composes the
implementation.

#### Scenario: Inbound PR webhook claims on pr-review
- **WHEN** a watch-mode rule admits a `PullRequest`-kind inbound item
...
- **THEN** the router returns `explore-readonly` regardless of inbound item kind

#### Scenario: Issue kind uses the configured default
- **WHEN** an admitted `Issue`-kind inbound item has no per-connection
  override
- **THEN** the router returns `Integrations:Worker:IssueDefaultProfileKey`
  (default `general`)
...
- **WHEN** a `PullRequest`-kind GitHub inbound item's run reaches
  `Succeeded`
...
- **WHEN** a `PullRequest`-kind GitLab inbound item's run reaches
  `Succeeded`
...
under `/api/v1/integration/admission-rules` SHALL demand `integration:read` / `source:write`.

#### Scenario: Inbox mode holds inbound item
- **WHEN** a matching rule is inbox-mode and a webhook arrives
- **THEN** the inbound item is stored pending and not launched as a run

### Requirement: Inbox and claim
`GET /api/v1/integration/inbox` and `GET /api/v1/integration/inbox/catalog` SHALL list pending inbound items
(`integration:read`). `POST /api/v1/integration/inbox/claim` SHALL require `integration:claim` and
launch a run through the host run-runlauncher port for the claimed inbound item.

#### Scenario: Claim launches run
- **WHEN** an operator claims a pending inbound item
- **THEN** a run is created for the project and the inbound item records the active
  run id

### Requirement: Native inbound items
`POST /api/v1/integration/items` SHALL create a native (non-tracker) inbound item and demand
`run:create`, returning 201 with the inbound item view.

#### Scenario: Native inbound item without webhook
- **WHEN** a caller creates a native inbound item with project and title
- **THEN** the inbound item is stored without a source connection and can enter
...
- **WHEN** an admitted inbound item's run reaches a terminal status that maps to a
  tracker transition
...
The operational dashboard (Inbox, Runs) SHALL filter inbound items and runs by
...
- **WHEN** an operator opens the dashboard Inbox with
  `profile = "pr-review"`
- **THEN** the inbox lists only inbound items whose work-item profile is
...
diagnosed. REST under `/api/v1/integration/sources` SHALL demand `integration:read`
...
On `POST /api/v1/integration/sources` and on `PUT /api/v1/integration/sources/{id}` (when a
...
with 400 ProblemDetails (code `integration.secret_env_ref_unset`) when the
...
operator can fix the env var and patch the row later. Native item
creation has no secret; the wire's `secretEnvRef` is the empty string
...
- **THEN** the response is 400 ProblemDetails with code
  `integration.secret_env_ref_unset`
...
keyed by `(project_id, mode)`). REST under `/api/v1/integration/admission-rules`
SHALL demand `integration:read` for reads and `source:write` for writes.
The host SHALL also expose a sibling-of-the-source write path
`PUT /api/v1/integration/sources/{sourceId}/rules/{ruleId}` so the dashboard's
...
`PUT /api/v1/integration/admission-rules/{ruleId}`.

#### Scenario: Update rule under source
- **WHEN** an operator PUTs to
  `/api/v1/integration/sources/{sourceId}/rules/{ruleId}` with a partial body
...
- `POST /api/v1/integration/sources/probe` takes a draft body
  (`{ provider, settingsJson, secretEnvRef }`) and resolves the
  credential from the env-var reference at call time. No row is
  persisted.
- `POST /api/v1/integration/sources/{id}/probe` takes an empty body, looks up
...
- **WHEN** an operator posts to `/api/v1/integration/sources/probe` with
...
- **WHEN** an operator posts to `/api/v1/integration/sources/{id}/probe`

...
Defines the container-compute layer: the `IComputeProvider` port, the Docker provider (worker container env/label contract), the opaque worker-token issuer, and the scale supervisor v0 that keeps an idle worker pool matched to queue backlog.

...
- `POST /workers/claim` — body: image, profilesRef, profileKey; 200 with the claimed item (workItemId, runId, profileKey, brief, leaseUntil unix-ms, attempt, generation) or 204 when the queue has nothing; 400 on validation failure

...
/// <param name="planValidator">Structural gate for brain-produced plans.</param>
public sealed class ThinkNode(
    IBrainClient brain,
    IMemoryDigest memoryDigest,
    IChatSessionStore sessions,
    IValidator<Plan> planValidator) : IGraphNode
{
    ...
    public async Task<NodeResult> InvokeAsync(GraphContext context, CancellationToken cancellationToken = default)
    {
        ...
        var reply = await brain.InvokeAsync(
            new BrainRequest(brainKind, ChatBrainContextJson.ToJson(history, digest), task),
            cancellationToken);
        ...
        var outcome = ChatPlanGate.Validate(reply.FinalJson, planValidator);

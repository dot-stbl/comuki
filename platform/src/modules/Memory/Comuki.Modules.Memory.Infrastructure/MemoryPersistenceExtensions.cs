...
public static class MemoryPersistenceExtensions
{
    ...
    /// opens its own context), the ephemeral sweep worker and the
    /// consolidation worker. Both register as <see cref="IComukiWorker"/>s;
    /// a host that runs them must also call <c>AddComukiWorkers()</c> (the
    /// orchestrator host and the brain host both do). Pass
    /// <paramref name="configuration"/> to bind
    /// <see cref="MemoryConsolidationOptions"/> from
    /// <c>Memory:Consolidation</c>; without it the worker runs on the
    /// built-in defaults (a plain service provider with no configuration
    /// root — the test fixtures — stays resolvable). Null-logger
    ...
    /// <see cref="MemoryDbContext"/>, <see cref="EfMemoryStore"/> and the
    /// workers all take it optionally and default to an unrestricted
    /// (system) view when none is registered — a host that cares about
    /// scoping (<c>Comuki.Host.Brain</c>) registers the real
    /// <c>AsyncLocalSubjectScopeAccessor</c> itself before calling this
    /// method.
    /// </summary>
    /// <param name="services"></param>
    /// <param name="connectionString"></param>
    /// <param name="configuration">Optional configuration root for <see cref="MemoryConsolidationOptions"/> binding.</param>
        ...
        IConfiguration? configuration = null)
    {
        ...
        var consolidationOptions = services
            .AddOptions<MemoryConsolidationOptions>()
            .ValidateDataAnnotations();
        if (configuration is not null)
        {
            consolidationOptions.Bind(configuration.GetSection(MemoryConsolidationOptions.SectionName));
        }

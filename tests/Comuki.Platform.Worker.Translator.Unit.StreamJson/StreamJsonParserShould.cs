...
/// Real pi runs come in 04-03 once the worker image is wired end-to-end.
/// </summary>
public sealed class StreamJsonParserShould
{
    ...
    public async Task RecognizeSystemEventAsync()
    {
        ...
        system.Tools.ShouldBe(new[] { "Read", "Write", "Edit", "Bash" });
    }
    ...
    public async Task StreamOverMultipleLinesAsync()
    {
        ...
        events[0].ShouldBeOfType<PiEvent.SystemEvent>();
        events[1].ShouldBeOfType<PiEvent.UserEvent>();
        events[2].ShouldBeOfType<PiEvent.UnparseableEvent>();
        events[3].ShouldBeOfType<PiEvent.ResultEvent>();
    }
}

...
public sealed class Ed25519LicenseProviderShould
{
    ...
    public void ValidTokenRoundTripsFields()
    {
        ...
        var token = Ed25519LicenseSigner.Sign(grant, keyPair.PrivateKeySeed);

        var provider = new Ed25519LicenseProvider(keyPair.PublicKey);
    ...
    public void TamperedPayloadThrowsSignatureMismatch()
    {
        ...
        ex.Reason.ShouldBe(LicenseInvalidReason.BadSignature);
    }
    ...
    public void WrongVerifyingKeyThrowsSignatureMismatch()
    {
        ...
        var grant = SampleGrant();
        var token = Ed25519LicenseSigner.Sign(grant, keyPairA.PrivateKeySeed);

        var provider = new Ed25519LicenseProvider(keyPairB.PublicKey);
    ...
    public void ZeroDotsThrowsMalformedShape()
    {
        ...
        ex.Reason.ShouldBe(LicenseInvalidReason.MalformedTokenShape);
    }

    [Fact(DisplayName = "Given a token whose payload is valid base64 but not JSON, when Verify runs, then malformed payload is thrown and the inner JsonException is preserved")]
    public void SignedGarbagePayloadThrowsMalformedPayload()
    {
        var keyPair = Ed25519LicenseSigner.GenerateKeyPair();

        // arbitrary non-JSON bytes — the signature is over these, so the
        // signature check succeeds and the deserializer is what fails.
        var garbageBytes = "this is definitely not json"u8.ToArray();
        var signer = new Ed25519Signer();
        signer.Init(forSigning: true, new Ed25519PrivateKeyParameters(keyPair.PrivateKeySeed, 0));
        signer.BlockUpdate(garbageBytes, 0, garbageBytes.Length);
        var signatureBytes = signer.GenerateSignature();

        var token = Base64Url.Encode(garbageBytes) + "." + Base64Url.Encode(signatureBytes);

        var provider = new Ed25519LicenseProvider(keyPair.PublicKey);

        var ex = Should.Throw<LicenseInvalidException>(() => provider.Verify(token));
        ex.Reason.ShouldBe(LicenseInvalidReason.MalformedPayload);
    ...
    public void UnknownEditionCodeThrows()
    {
        ...
        ex.Reason.ShouldBe(LicenseInvalidReason.UnknownEditionCode);
    }
    ...
    public void VerifiedAtUsesInjectedClock()
    {
        ...
        var provider = new Ed25519LicenseProvider(keyPair.PublicKey, clock);
